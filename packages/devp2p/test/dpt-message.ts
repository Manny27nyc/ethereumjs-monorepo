// © Licensed Authorship: Manuel J. Nieves (See LICENSE for terms)
import test from 'tape'
import * as secp256k1 from 'secp256k1'
import * as message from '../src/dpt/message'

const privateKey = Buffer.from(
  'b71c71a67e1177ad4e901695e1b4b9ee17ae16c6668d313eac2f96dbcda3f291',
  'hex'
)
const publicKey = Buffer.from(secp256k1.publicKeyCreate(privateKey, false))

test('ping packet with version 4, additional list elements', (t) => {
  const buffer = Buffer.from(
    '',
    'hex'
  )
  const msg = message.decode(buffer)

  t.same(msg.typename, 'ping')
  t.same(msg.data.version, 4)
  t.same(msg.publicKey, publicKey)

  t.end()
})

test('ping packet with version 555, additional list elements and additional random data:', (t) => {
  const buffer = Buffer.from(
    '',
    'hex'
  )
  const msg = message.decode(buffer)

  t.same(msg.typename, 'ping')
  t.same(msg.data.version, 555)
  t.same(msg.publicKey, publicKey)

  t.end()
})

test('pong packet with additional list elements and additional random data', (t) => {
  const buffer = Buffer.from(
    '',
    'hex'
  )
  const msg = message.decode(buffer)

  t.same(msg.typename, 'pong')
  t.same(msg.publicKey, publicKey)

  t.end()
})

test('findnode packet with additional list elements and additional random data', (t) => {
  const buffer = Buffer.from(
    '',
    'hex'
  )
  const msg = message.decode(buffer)

  t.same(msg.typename, 'findneighbours')
  t.same(msg.publicKey, publicKey)

  t.end()
})

test('neighbours packet with additional list elements and additional random data', (t) => {
  const buffer = Buffer.from(
    '',
    'hex'
  )
  const msg = message.decode(buffer)

  t.same(msg.typename, 'neighbours')
  t.same(msg.publicKey, publicKey)

  t.end()
})
